...
class FCCWizard(QtCore.QObject):
    ...
    def __init__(self, ui, analyser, main_window):
        super().__init__()
        self.ui = ui
    ...
    def _require_usb(self) -> bool:
        if self.analyser.usb is None or not self.analyser.usb.is_open:
            QMessageBox.critical(self.ui, "TinySA", "TinySA not connected. Connect the device first.")
            return False
        return True

    def _take_serial_control(self):
        """
        Stop the main QtTinySA sweep/FIFO so the wizard can talk on the same COM port.
        Without this, concurrent scanraw + serialWrite causes Windows:
        WriteFile failed (PermissionError(13), 'The device does not recognize the command.').
        """
        a = self.analyser
        a.sweeping = False
        try:
            if hasattr(a, "fifoTimer") and a.fifoTimer.isActive():
                a.fifoTimer.stop()
        except Exception:
            pass
        # Abort any in-flight scanraw so the worker exits promptly
        try:
            if a.usb and a.usb.is_open:
                a.abort()
        except Exception:
            pass
        for _ in range(80):
            if not a.threadRunning:
                break
        ...
        try:
            ...
            pass
        # Drain any leftover FIFO commands without sending while we own the port
        try:
            while a.fifo.qsize() > 0:
                a.fifo.get_nowait()
        except Exception:
            pass
        self._serial_held = True

    def _release_serial_control(self):
        """Restart main-app FIFO timer if we stopped it for wizard commands."""
        a = self.analyser
        if not getattr(self, "_serial_held", False):
            return
        try:
            if hasattr(a, "fifoTimer") and not a.fifoTimer.isActive():
                a.fifoTimer.start(200)
        except Exception:
            pass
        self._serial_held = False

    def _safe_write(self, cmd: str, retries: int = 2):
        a = self.analyser
        last = None
    ...
    def _safe_query(self, cmd: str) -> str:
        a = self.analyser
        a.clearBuffer()
        return a.serialQuery(cmd)

    def _clear_maxhold_buffer(self):
        """Turn off device calc mode so the previous max-hold trace cannot bleed into the next run."""
        a = self.analyser
        if a.usb is None or not a.usb.is_open:
            return
        try:
            self._take_serial_control()
            self._safe_write("pause\r")
            self._safe_write("calc off\r")
            time.sleep(0.1)
    ...
    def _verify_maxhold(self) -> bool:
        time.sleep(1.0)
        r1 = parse_float_lines(self._safe_query("data 2\r"))
        time.sleep(2.0)
        r2 = parse_float_lines(self._safe_query("data 2\r"))
    ...
    def _measure_sweep_time(self):
        ...
        try:
            self._take_serial_control()
            self._safe_write("pause\r")
            self._safe_write("ultra on\r")
            self._safe_write(f"sweep start {START_FREQ_HZ}\r")
            self._safe_write(f"sweep stop {STOP_FREQ_HZ}\r")
            self._safe_write(f"rbw {RBW_KHZ}\r")
            self._safe_write("resume\r")
            t0 = time.time()
            time.sleep(0.5)
            self._safe_query("data 2\r")
            self._safe_query("data 2\r")
    ...
    def _stop_main_sweep(self):
        """Compat shim — prefer _take_serial_control."""
        self._take_serial_control()
    ...
    def _quiesce_app(self):
        ...
        a.ports = []
        self._quiesced = True
        self._serial_held = False  # port closed; FIFO restart happens in _restore_app
    ...
    def _restore_app(self):
        ...
        if ports and a.usb is None:
            try:
                a.testPort(ports[0])
                if ports[0] not in a.ports:
                    a.ports.append(ports[0])
            except Exception as e:
                logging.info(f"restore reconnect: {e}")
        self._quiesced = False
        self._serial_held = False
    ...
    def _on_replugged(self, port):
        ...
        try:
            a.testPort(port)
            if port not in a.ports:
                a.ports.append(port)
        except Exception as e:
            QMessageBox.critical(self.ui, "Reconnect failed", str(e))
            self.poll_timer.start()
            return
    ...
    def _read_trace(self):
        a = self.analyser
        if not a.usb or not a.usb.is_open:
            raise RuntimeError("Serial port not open after reconnect")
        u = self.ui
        a.serialWrite("pause\r")
        time.sleep(0.2)
        u.dumpStatus.setText("Reading frequencies…")
        QtCore.QCoreApplication.processEvents()
        freqs = parse_float_lines(a.serialQuery("frequencies\r"))
        u.dumpStatus.setText("Reading data 2 (max hold)…")
        QtCore.QCoreApplication.processEvents()
        vals = parse_float_lines(a.serialQuery("data 2\r"))
